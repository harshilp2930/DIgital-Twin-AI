...
def contact():
    if request.method == 'POST':
        ...
        flash(f'Thank you {full_name}! Your message has been received. We\'ll get back to you shortly.', 'success')
...
@app.route('/user/financial/edit/<int:id>', methods=['POST'])
@login_required
...
@app.route('/user/financial/delete/<int:id>', methods=['POST'])
@login_required
...
@app.route('/user/study/edit', methods=['POST'])
@login_required
def edit_study_log():
    ...
    try:
        ...
        db.session.rollback()
        flash(f'Error updating study log: {str(e)}', 'danger')
    return redirect(url_for('user_study'))


@app.route('/user/study/delete/<int:id>', methods=['POST'])
@login_required
def delete_study_log(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(f'Error deleting study log: {str(e)}', 'danger')
    return redirect(url_for('user_study'))
    ...
    if request.method == 'POST':
        ...
        if gemini_model and GEMINI_API_KEY:
            try:
                ...
                flash('AI service unavailable. Using fallback.', 'warning')
                response = None
